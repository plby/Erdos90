...
def normalizedAdicHom
  ...
  toFun x :=
    Submission.NumberTheory.Milne.normalizedAdicOrder p x.toMul
  map_zero' := by
    simp [Submission.NumberTheory.Milne.normalizedAdicOrder]
  map_add' x y := by
    ...
    simp only [Submission.NumberTheory.Milne.normalizedAdicOrder]
    abel
...
theorem normalized_adic_surjective
  ...
  intro z
  obtain ⟨x, hx⟩ :=
    Submission.NumberTheory.Milne.normalized_order_surjective
      p (K := K) z
...
theorem algebra_ramification_idx
  ...
  exact
    Submission.NumberTheory.Milne.normalized_adic_ramification
