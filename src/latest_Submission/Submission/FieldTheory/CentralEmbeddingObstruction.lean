...
theorem cyclic_unramified_completion
  ...
  let v := (FinitePlace.mk P).val
  let hv := Submission.CField.Ideles.absolute_value_nontrivial P
  let hvna : IsNonarchimedean v := fun x y ↦ (FinitePlace.mk P).add_le x y
  letI : Fact v.IsNontrivial := ⟨hv⟩
  letI : IsUltrametricDist v.Completion :=
    Submission.CField.Ideles.placeUltrametricDist P
  let hw := Submission.NumberTheory.Milne.absolute_extension_nontrivial
    v w
  let hwna :=
    Submission.NumberTheory.Milne.absolute_extension_nonarchimedean
      v w
  let Q := Submission.NumberTheory.Milne.nonarchimedeanHeightSpectrum
    w.1 hw hwna
  ...
  have hDecompositionCyclic :
    ...
    Submission.NumberTheory.Milne.decomposition_cyclic_unramified
      P Q hQ
  have hAbsoluteCyclic :
      IsCyclic
        (Submission.NumberTheory.Milne.absoluteValueDecomposition
          v w.1) := by
    rw [← Submission.NumberTheory.Milne.centered_stabilizer_decomposition
      v w.1 hw hwna]
  ...
  let A₀ := Valuation.integer (ValuativeRel.valuation v.Completion)
  let A := Submission.NumberTheory.Milne.completionIntegerRing v
  let B := Submission.NumberTheory.Milne.completionIntegerRing w.1
  letI : Algebra A B :=
    Submission.NumberTheory.Milne.completionIntegerLies
      v w.1 w.2
  let e₀ : A₀ ≃+* A :=
    Submission.CField.LBrauer.valuativeIntegerNorm
      v.Completion
  letI : Algebra A₀ B :=
  ...
  let eA :=
    Submission.NumberTheory.Milne.centeredIntegerAdic
      v hv hvna
  ...
  let hModuleFiniteA : Module.Finite A B :=
    Submission.NumberTheory.Milne.completion_integer_module
  ...
  let hUnramifiedA : Algebra.FormallyUnramified A B :=
    Submission.NumberTheory.Milne.completion_formally_unramified
  ...
  let hLocalHomA : IsLocalHom (algebraMap A B) :=
    Submission.NumberTheory.Milne.completion_integer_lies
      v w.1 w.2
