...
import Submission.Algebra.OnePlusIdeal
...
theorem augmentation_ideal_algebra :
    Submission.GShafar.augmentationIdeal R (FreeGroup X) =
      Submission.GroupAlgebra.augmentationIdeal R (FreeGroup X) := by
  unfold Submission.GShafar.augmentationIdeal
    Submission.GroupAlgebra.augmentationIdeal
  congr 1
...
noncomputable def augmentationAddFiltration :
  ...
  term n :=
    (Submission.GShafar.augmentationIdeal R (FreeGroup X) ^ n).toAddSubgroup
  antitone := by
    intro m n hmn
...
theorem augmentation_order_least :
  ...
  rw [Ideal.map_le_iff_le_comap, augmentation_ideal_algebra,
    ← Submission.GroupAlgebra.augmentation_generator_ideal]
...
theorem magnus_least_pow
    ...
    (hf :
      f ∈ Submission.GShafar.augmentationIdeal R (FreeGroup X) ^ i) :
  ...
  exact order_least_pow (R := R) (X := X) i
    (Submission.GroupAlgebra.map_le_le
      (groupAlgebraMagnus (R := R) (X := X))
      (Submission.GShafar.augmentationIdeal R (FreeGroup X))
      (MSeries.orderLeastIdeal (R := R) (X := X) 1)
