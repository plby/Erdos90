...
private instance sqrt_no_divisors :
    NoZeroDivisors Submission.NumberTheory.SNFive where
...
private instance sqrt_five_domain :
    IsDomain Submission.NumberTheory.SNFive :=
  NoZeroDivisors.to_isDomain _

private def negFiveEmbedding :
    Submission.NumberTheory.SNFive →+* QFModel (-5) where
...
private local instance :
    Algebra Submission.NumberTheory.SNFive (QFModel (-5)) :=
  negFiveEmbedding.toAlgebra

private local instance :
    IsScalarTower ℤ Submission.NumberTheory.SNFive (QFModel (-5)) :=
...
private local instance :
    Ring.HasFiniteQuotients Submission.NumberTheory.SNFive :=
...
private theorem sqrt_five_bot :
  ...
  intro hbot
  have hmem : (2 : Submission.NumberTheory.SNFive) ∈
      SNFive.primeIdealTwo := by
...
private theorem sqrt_or_abs
    (I : Ideal Submission.NumberTheory.SNFive)
  ...
  · right
    ...
    letI : Nontrivial
        (Submission.NumberTheory.SNFive ⧸ I) :=
      Ideal.Quotient.nontrivial_iff.mpr hItop
    have hcard : Nat.card
        (Submission.NumberTheory.SNFive ⧸ I) = 2 := by
      simpa [Ideal.absNorm_apply, Submodule.cardQuot_apply] using hnorm
    have htwo : (2 : Submission.NumberTheory.SNFive) ∈ I := by
      simpa [hnorm] using Ideal.absNorm_mem I
    let q : Submission.NumberTheory.SNFive ⧸ I :=
      Ideal.Quotient.mk I (⟨0, 1⟩ : Submission.NumberTheory.SNFive)
    have hq_sq : q ^ 2 = 1 := by
      have hminusfive :
          Ideal.Quotient.mk I (-5 : Submission.NumberTheory.SNFive) = 1 := by
        apply Ideal.Quotient.eq.mpr
        convert I.mul_mem_left
          (-3 : Submission.NumberTheory.SNFive) htwo using 1
      calc
        q ^ 2 = Ideal.Quotient.mk I
            ((⟨0, 1⟩ : Submission.NumberTheory.SNFive) ^ 2) := by
              rfl
        _ = Ideal.Quotient.mk I
            (-5 : Submission.NumberTheory.SNFive) := by
              congr 1
        _ = 1 := hminusfive
    have hq0 : q ≠ 0 := by
      intro hq
    ...
    have hq1 : q = 1 := by
      obtain ⟨y, hy0, hyuniq⟩ := (Nat.card_eq_two_iff' (0 :
        Submission.NumberTheory.SNFive ⧸ I)).mp hcard
      exact (hyuniq q hq0).trans (hyuniq 1 one_ne_zero).symm
    have honeAdd :
        (⟨1, 1⟩ : Submission.NumberTheory.SNFive) ∈ I := by
      rw [← Ideal.Quotient.eq_zero_iff_mem]
      calc
        Ideal.Quotient.mk I
            (⟨1, 1⟩ : Submission.NumberTheory.SNFive) = 1 + q := by
              rfl
        _ = 1 + 1 := by rw [hq1]
        _ = Ideal.Quotient.mk I
            (2 : Submission.NumberTheory.SNFive) := by rfl
...
private def quadraticSqrtFive :
    QOrd (-5) 0 ≃ₐ[ℤ] Submission.NumberTheory.SNFive :=
...
private theorem sqrt_five_discr :
  ...
  let eAlg : Submission.NumberTheory.SNFive ≃ₐ[ℤ]
      𝓞 (QFModel (-5)) :=
    ...
    _ = Algebra.discr ℤ sqrtFiveBasis := by
      simpa [b'] using
        (Algebra.discr_eq_discr_of_algEquiv
          (sqrtFiveBasis : Fin 2 →
            Submission.NumberTheory.SNFive) eAlg).symm
...
noncomputable def sqrtFiveNonzero :
    (Ideal (Submission.NumberTheory.SNFive))⁰ :=
...
noncomputable def sqrtFiveNontrivial :
    ClassGroup Submission.NumberTheory.SNFive :=
...
theorem negative_quadratic_five :
  ...
  have hall : ∀ C : ClassGroup (𝓞 (QFModel (-5))), C = 1 ∨ C = c := by
    ...
    have hnormNat :
      ...
      omega
    let J : Ideal Submission.NumberTheory.SNFive := (I : Ideal _).map e
    have hJne : J ≠ ⊥ := by
...
theorem sqrt_five_two :
    Nat.card (ClassGroup Submission.NumberTheory.SNFive) = 2 := by
  let E := classGroupRing integersSqrtFive
  calc
    Nat.card (ClassGroup Submission.NumberTheory.SNFive) =
...
theorem sqrt_five_not
    {I J : Ideal Submission.NumberTheory.SNFive}
    (hI0 : I ≠ ⊥) (hJ0 : J ≠ ⊥)
    (hI : ¬ I.IsPrincipal) (hJ : ¬ J.IsPrincipal) :
    (I * J).IsPrincipal :=
  ideal_principal_two
