...
import Submission.NumberTheory.HigherRamification
...
theorem higher_residue_additive
  ...
  exact Submission.lift_injective_ker N psi hN_le hker
...
instance wildInertiaNormal
    ...
    (Submission.number_wild_subgroup (L := L) P).Normal := by
  rw [← Submission.higher_ramification_one (L := L) P]
  exact Submission.number_higher_normal (L := L) P 1
...
abbrev numberInertiaWild
    ...
    [FiniteDimensional ℚ L] [IsGalois ℚ L]
    (P : Ideal (NumberField.RingOfIntegers L)) :=
  P.inertia (Gal(L/ℚ)) ⧸ Submission.number_wild_subgroup (L := L) P
...
theorem number_wild_units
  ...
  let N := Submission.number_wild_subgroup (L := L) P
  obtain ⟨χ, hχ⟩ :=
    Submission.tame_uniformizer_wild
      (L := L) hq P
  have hN_le : N ≤ χ.ker := by
    rw [hχ]
  refine ⟨QuotientGroup.lift N χ hN_le, ?_⟩
  exact Submission.lift_injective_ker N χ hN_le hχ
...
theorem number_higher_additive
    ...
    [P.IsPrime] [P.LiesOver (Ideal.rationalPrimeIdeal q)]
    (i : ℕ) (hi : 1 ≤ i) :
    ∃ φ : Submission.number_ramification_step (L := L) P i →*
        Multiplicative P.ResidueField,
      Function.Injective φ :=
  Submission.higher_ramification_additive
    (L := L) hq P i hi
...
theorem number_inertia_solvable
  ...
  have hNfinite : Finite N :=
    Submission.number_field_wild (L := L) hq P
  letI : Finite N := hNfinite
  have hNp : IsPGroup q N :=
    Submission.number_wild_group (L := L) hq P
...
theorem decomposition_group_solvable
  ...
  have hPmax : P.IsMaximal :=
    Submission.number_above_maximal (L := L) hq P
  ...
  have hquotCyclic : IsCyclic (D ⧸ ID) := by
    simpa [D, ID, I] using
      (Submission.decomposition_inertia_cyclic (L := L) hq P)
...
theorem number_inertia_bot
  ...
  have hcard :
    ...
    Submission.inertia_ramification_idx (L := L) hq P
  constructor
  · intro he
...
theorem coprime_wild_bot
    ...
    Nat.Coprime q
        (Ideal.ramificationIdx (Ideal.rationalPrimeIdeal q) P) ↔
      Submission.number_wild_subgroup (L := L) P = ⊥ := by
  classical
  letI : Fact q.Prime := ⟨hq⟩
  letI : Finite (Gal(L/ℚ)) :=
    IsGaloisGroup.finite (Gal(L/ℚ)) ℚ L
  let I := P.inertia (Gal(L/ℚ))
  let W := Submission.number_wild_subgroup (L := L) P
  ...
  · intro he
    have heI : Nat.Coprime q (Nat.card I) := by
      rw [hcard]
      exact he
    apply Submission.bot_coprime_card W hWp
    exact heI
  · intro hW
    obtain ⟨χ, hχ⟩ :=
      Submission.tame_uniformizer_wild
    ...
    letI : CharP P.ResidueField q :=
      Submission.residue_char_p (L := L) hq P
    letI : Finite P.ResidueField :=
      Submission.number_local_residue (L := L) hq P
