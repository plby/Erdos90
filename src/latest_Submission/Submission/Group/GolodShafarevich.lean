...
private lemma left_invariant_mul {p : ℕ} {G : Type u} [Group G]
    ...
    φ ⟨a * (x : MonoidAlgebra (ZMod p) G), J.mul_mem_left a x.2⟩ =
      (Submission.GroupAlgebra.augmentation (ZMod p) G a) * φ x := by
  classical
  let L : MonoidAlgebra (ZMod p) G →ₗ[ZMod p] ZMod p :=
    φ.comp (leftIdealLinear J x)
  let R : MonoidAlgebra (ZMod p) G →ₗ[ZMod p] ZMod p :=
  { toFun := fun a => (Submission.GroupAlgebra.augmentation (ZMod p) G a) * φ x
    map_add' := by intro a b; simp [map_add, add_mul]
    map_smul' := by
      intro r a
      simp [_root_.Algebra.smul_def, mul_assoc, Submission.GroupAlgebra.augmentation,
        Submission.GroupAlgebra.trivialCharacter] }
  have hLR : L = R := by
    ext g
    simpa [L, R, leftIdealLinear, leftRegularLinear,
      Submission.GroupAlgebra.augmentation, Submission.GroupAlgebra.trivialCharacter] using hinv g x
...
private lemma ne_self_bot {p : ℕ} {G : Type u}
    ...
    (J : Ideal (MonoidAlgebra (ZMod p) G)) (hJne : J ≠ ⊥) :
    Submission.Algebra.augmentationIdeal (ZMod p) G * J ≠ J := by
  classical
  obtain ⟨φ, hφne, hinv⟩ :=
  ...
  { carrier := {a | ∃ ha : a ∈ J, φ ⟨a, ha⟩ = 0}
    ...
    smul_mem' := by
      ...
      simpa [hφa] using h }
  have hIJleK : Submission.Algebra.augmentationIdeal (ZMod p) G * J ≤ K := by
    refine Ideal.mul_le.mpr ?_
    intro a ha b hb
    refine ⟨J.mul_mem_left a hb, ?_⟩
    have haug : Submission.GroupAlgebra.augmentation (ZMod p) G a = 0 := by
      simpa [Submission.Algebra.augmentationIdeal, Submission.GroupAlgebra.augmentationIdeal] using ha
    have h :=
  ...
  have hJleK : J ≤ K := by
    intro a ha
    have ha' : a ∈ Submission.Algebra.augmentationIdeal (ZMod p) G * J := by
      simpa [hEq] using ha
    exact hIJleK ha'
...
private lemma pow_augmentation_bot {p : ℕ} {G : Type u}
    [Group G] [Fintype G] (hp : Nat.Prime p)
    (hcardG : ∃ n : ℕ, Fintype.card G = p ^ n) :
    ∃ n : ℕ, (Submission.Algebra.augmentationIdeal (ZMod p) G) ^ n = ⊥ := by
  classical
  letI : Fact (Nat.Prime p) := ⟨hp⟩
  letI : NeZero p := ⟨hp.ne_zero⟩
  letI : DecidableEq G := Classical.typeDecidableEq G
  ...
  let I : Ideal A := Submission.Algebra.augmentationIdeal (ZMod p) G
  haveI : I.IsTwoSided := by
    dsimp [I, Submission.Algebra.augmentationIdeal, Submission.GroupAlgebra.augmentationIdeal]
    infer_instance
...
theorem nilpotencePGroups {p : ℕ} {G : Type u} [Group G]
  ...
  dsimp [N]
  haveI : (Submission.Algebra.augmentationIdeal (ZMod p) G).IsTwoSided := by
    dsimp [Submission.Algebra.augmentationIdeal, Submission.GroupAlgebra.augmentationIdeal]
    infer_instance
  have hpows :
      Submission.Algebra.augmentationIdeal (ZMod p) G ^ (n0 + d) =
        Submission.Algebra.augmentationIdeal (ZMod p) G ^ n0 *
          Submission.Algebra.augmentationIdeal (ZMod p) G ^ d :=
    Ideal.IsTwoSided.pow_add (I := Submission.Algebra.augmentationIdeal (ZMod p) G) n0 d
  change Submission.Algebra.augmentationIdeal (ZMod p) G ^ (n0 + d) = ⊥
  rw [hpows]
  rw [show Submission.Algebra.augmentationIdeal (ZMod p) G ^ n0 = ⊥ by
