...
import Submission.FieldTheory.CyclotomicCleanupAssembly
import Submission.FieldTheory.FinalCleanupAssembly
import Submission.FieldTheory.ConductorNineAssembly
import Submission.ClassField.GrunwaldWang.IdeleCharacterGlobalization
...
theorem integer_primitive_dvd
  ...
  let a0 : k := g0
  let a : B :=
    Submission.NumberTheory.Milne.teichmullerLift B a0
  let q := Fintype.card k
  have haq : a ^ q = a := by
    exact sub_eq_zero.mp (by
      simpa [a, q, Polynomial.IsRoot.def] using
        Submission.NumberTheory.Milne.teichmuller_lift_root B a0)
  have haPowRoot :
  ...
  have haResiduePow :
    ...
    rw [show IsLocalRing.residue B a = a0 by
      exact Submission.NumberTheory.Milne.residue_teichmullerLift B a0]
    change ((g0 : k) ^ n) = 1
  ...
  have haPow : a ^ n = 1 :=
    Submission.NumberTheory.Milne.teichmullerLift_unique B
      haPowRoot hOneRoot haResiduePow
  have haUnit : IsUnit a := by
    rw [← IsLocalRing.residue_ne_zero_iff_isUnit]
    rw [Submission.NumberTheory.Milne.residue_teichmullerLift]
  ...
  · have hmap := orderOf_map_dvd (IsLocalRing.residue B).toMonoidHom a
    ...
    have hresidue : IsLocalRing.residue B a = a0 :=
      Submission.NumberTheory.Milne.residue_teichmullerLift B a0
...
theorem cube_stabilizer_bot
  ...
  have hcard :=
    Submission.NumberTheory.Milne.decomposition_inertia_deg
...
theorem compositum_conjugate_completion
  ...
  have hna : IsNonarchimedean w.1 :=
    Submission.NumberTheory.Milne.absolute_extension_nonarchimedean v w
  let Qs :=
    Submission.NumberTheory.Milne.nonarchimedeanHeightSpectrum
      w.1 hw hna
...
noncomputable def absoluteExtensionNonarchimedean
  ...
  letI : NontriviallyNormedField w.Completion :=
    Submission.NumberTheory.Milne.absoluteNontriviallyNormed w
