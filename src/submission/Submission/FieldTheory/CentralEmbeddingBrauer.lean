...
import Submission.ClassField.GrunwaldWang.CyclicDivision
...
theorem inflation_coefficients_restriction
    ...
    Submission.CField.CProduca.inflationHom K hFM x =
...
theorem h_inflates_brauer
    ...
    (x : MHTwo Gal(L/K) Lˣ)
    (hx : Submission.CField.CProduca.finiteHBrauer K L x = 1) :
    ∃ E : FiniteGaloisIntermediateField K (SeparableClosure K),
      ∃ hLE : L ≤ E,
        Submission.CField.CProduca.inflationHom K hLE x = 1 := by
  open Submission.CField.CProduca in
  ...
  let y : Σ F,
      Submission.CField.CProduca.multiplicativeHFamily K F :=
    ⟨L, x⟩
  change (⟦y⟧ :
      Submission.CField.CProduca.absoluteMultiplicativeH K) = 1
    at habsolute
...
theorem central_inflation_lift
    ...
    Submission.CField.CProduca.inflationHom K hFM
...
noncomputable def restrictedGaloisCocycle
  ...
  let D := Submission.NumberTheory.Milne.completionDecompositionField
    v hvna w
  let H := Submission.NumberTheory.Milne.completionDecompositionGroup
    v hvna w
  ...
  let W := Submission.CField.ICohomo.CompletionPlacesAbove
    (L := L) v
  letI : Finite W :=
    Submission.NumberTheory.Milne.absolute_extensions_separable v
  letI : Nonempty W :=
    Submission.NumberTheory.Milne.absolute_value_extension
      (K := K) (L := L) v
  letI : MulAction.IsPretransitive Gal(L/K) W :=
    Submission.NumberTheory.Milne.above_pretr_nonar
      v hvna
  letI : Algebra v.Completion w.1.Completion :=
    (Submission.NumberTheory.Milne.completionLies
      v w.1 w.2).toAlgebra
  ...
  let cD : NMCocycl₂ (G := Gal(L/D)) (M := Lˣ) :=
    NMCocycl₂.restrict (galoisTowerInclusion K D L)
      (fun _ _ => rfl) c
  let g :=
    Submission.NumberTheory.Milne.decompositionGaloisCompletion
      v hvna w
  exact transportedGaloisCocycle
    (Submission.NumberTheory.Milne.completionEmbedding w.1) g
    (fun sigma a =>
...
noncomputable def completionKernelUnits
  ...
  (Units.map
    (Submission.NumberTheory.Milne.completionEmbedding w.1)).comp
      kernelToUnits
...
theorem completion_units_fixed
  ...
  apply Units.ext
  change sigma
      (Submission.NumberTheory.Milne.completionEmbedding w.1
        (kernelToUnits z : L)) =
    Submission.NumberTheory.Milne.completionEmbedding w.1
      (kernelToUnits z : L)
  ...
  calc
    sigma (Submission.NumberTheory.Milne.completionEmbedding w.1
        (kernelToUnits z : L)) =
        g tau (Submission.NumberTheory.Milne.completionEmbedding w.1
          (kernelToUnits z : L)) := by rw [g.apply_symm_apply sigma]
    _ = Submission.NumberTheory.Milne.completionEmbedding w.1
          (tau (kernelToUnits z : L)) :=
      Submission.NumberTheory.Milne.decomposition_galois_embedding
...
theorem base_change_crossed
  ...
  open Submission.NumberTheory.Milne in
...
noncomputable def completionTransportedCocycle
    ...
    (w : Submission.CField.ICohomo.CompletionPlacesAbove
      (L := L) v)
    (htop : Submission.NumberTheory.Milne.absoluteValueDecomposition
      v w.1 = ⊤)
    (c : NMCocycl₂ (G := Gal(L/K)) (M := Lˣ)) :
    letI : Algebra v.Completion w.1.Completion :=
      (Submission.NumberTheory.Milne.completionLies
  ...
  letI : IsGalois v.Completion w.1.Completion :=
    Submission.NumberTheory.Milne.placeCompletionGalois v w
  let g :=
    Submission.NumberTheory.Milne.globalDecompositionTop
      v w htop
  exact transportedGaloisCocycle
    (Submission.NumberTheory.Milne.completionEmbedding w.1) g
    (fun sigma a =>
...
theorem completion_embedding_base
    ...
    (Submission.NumberTheory.Milne.completionEmbedding v).toAlgebra =
      Submission.NumberTheory.Milne.completionBaseAlgebra v := by
...
theorem brauer_primitive_cube
    ...
    letI : Algebra K v.1.Completion :=
      Submission.NumberTheory.Milne.completionBaseAlgebra v.1
...
theorem central_all_completions
    ...
    (placeInvariant : ∀ place : NumberFieldPlace K,
      Additive (BrauerGroup (placeCompletion K place)) →+
        Submission.CField.LBrauer.LocalInvariant)
  ...
  | inr v =>
      change @brauerBaseChange K v.1.Completion
        inferInstance inferInstance
          (Submission.NumberTheory.Milne.completionEmbedding v.1).toAlgebra
          (extensionRelativeBrauer q hq hcentral galoisEquiv
            kernelToUnits hfixed : BrauerGroup K) = 1
...
theorem cyclic_brauer_splitter
    (K : Type u) [Field K] [NumberField K]
    (hcyclic :
      Submission.CField.GWang.DivisionCyclicityTheorem K)
    (x : BrauerGroup K) :
    ∃ L : Submission.CField.LFTheory.FASubext K,
