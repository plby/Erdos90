...
noncomputable def relativeBrauerSubgroup
    ...
    AddSubgroup (Additive (BrauerGroup (Submission.CField.RExist.placeCompletion K v))) := by
  cases v with
  | inl P =>
...
noncomputable def brauerLiftingChange
  ...
  | inl P =>
      exact @brauerBaseChange
        (Submission.CField.RExist.placeCompletion K (.inl P))
        (chosenCompletionExtension K L completion (.inl P))
        inferInstance inferInstance
        (chosenCompletionAlgebra K L completion (.inl P))
  | inr v =>
      exact @brauerBaseChange
        (Submission.CField.RExist.placeCompletion K (.inr v))
...
noncomputable def localBrauerInclusion
    ...
    Additive (localRelativeBrauer K L completion v) →+
      Additive (BrauerGroup (Submission.CField.RExist.placeCompletion K v)) := by
  cases v with
  | inl P =>
