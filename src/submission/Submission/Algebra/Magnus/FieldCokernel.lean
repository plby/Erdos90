...
noncomputable def scalarExtendedBasis
    ...
    (n : ℕ) :
    Module.Basis
      (Submission.HallTree.BasicIndex (α := X) (n + 1))
      K
...
noncomputable def scalarExtendedMagnus
  ...
  (Finsupp.linearCombination K
      (fun i : Submission.HallTree.BasicIndex (α := X) (n + 1) =>
        Submission.HallTree.associativeRepWeight K
          (Submission.HallTree.indexedBasicTree i)
          (Submission.HallTree.indexed_tree_weight i))).comp
...
theorem scalar_extended_magnus
    (n : ℕ)
    (i : Submission.HallTree.BasicIndex (α := X) (n + 1)) :
