import Towers.FieldTheory.TameThreeKoch.FiniteLocalObstructions
import Towers.FieldTheory.TameThreeKoch.AbsoluteLiftTransport
import Towers.FieldTheory.TameThreeKoch.CyclotomicCompatibility
import Towers.FieldTheory.TameThreeKoch.LiftProjectedInertia
...
private theorem brauer_all_completions
    ...
    (hlocal : ∀ place : NumberFieldPlace K,
      Towers.CField.BGroups.brauerBaseChange
        K (Towers.CField.Ideles.placeCompletion K place) beta = 1) :
    beta = 1 := by
  ...
  change (DirectSum.component ℤ (NumberFieldPlace K)
      (fun v ↦ Additive (BrauerGroup
        (Towers.CField.RExist.placeCompletion
          K v))) place)
  ...
  | inl P =>
      letI : Algebra K (FinitePlace.mk P).val.Completion :=
        (Towers.NumberTheory.Milne.completionEmbedding
      ...
      have hnormalized := brauer_change_alg
        K (P.adicCompletion K) (FinitePlace.mk P).val.Completion
          e.symm beta
          (by
            simpa only [Towers.CField.Ideles.placeCompletion] using
              hlocal (.inl P))
...
theorem rational_cft_solvable
  ...
  have hzero :
    ...
    have hlocal : ∀ place : NumberFieldPlace Kc,
        Towers.CField.BGroups.brauerBaseChange
          Kc (Towers.CField.Ideles.placeCompletion Kc place) beta = 1 := by
      intro place
      cases place with
      | inl P =>
          exact hfinite P
      | inr v =>
          change @Towers.CField.BGroups.brauerBaseChange
