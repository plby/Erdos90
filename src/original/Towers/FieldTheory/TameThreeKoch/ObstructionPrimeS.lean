import Towers.FieldTheory.TameThreeKoch.LocalObstructionHelpers
...
private theorem obstruction_order_smul
  ...
  letI : NontriviallyNormedField v.Completion :=
    Towers.CField.Ideles.placeNontriviallyNormed
      P
  ...
  letI : IsNonarchimedeanLocalField v.Completion :=
    Towers.CField.Ideles.placeNonarchimedeanField
      P
  letI : Fact (AbsoluteValue.LiesOver w.1 v) := ⟨w.2⟩
  letI : Algebra v.Completion w.1.Completion :=
    (Towers.NumberTheory.Milne.completionLies
  ...
  letI : NontriviallyNormedField w.1.Completion :=
    Towers.NumberTheory.Milne.absoluteNontriviallyNormed
      w.1
  ...
  · rw [local_order_valuation]
    ...
    have hnorm : ‖(z : w.1.Completion)‖ ≤
      ...
      rw [Towers.NumberTheory.Milne.norm_spectral_completion
          v w.1 w.2 inferInstance,
        Towers.NumberTheory.Milne.norm_spectral_completion
          v w.1 w.2 inferInstance]
...
private theorem obstruction_completed_zpowers
    ...
    (hgen0 : Subgroup.closure ({(tau0 : Gal(Lf/ℚ))} : Set _) =
      P0.inertia Gal(Lf/ℚ))
    (tauD : Towers.NumberTheory.Milne.absoluteValueDecomposition
      v w.1)
    (htauM : eComp tauD.1 = (AlgEquiv.autCongr eLayer) tau0)
    (heComp : ∀ zD :
      Towers.NumberTheory.Milne.absoluteValueDecomposition v w.1,
...
theorem cft_obstruction_s
  ...
  letI hIntegralAction : MulSemiringAction Gal(M/Kc)
      (NumberField.RingOfIntegers M) :=
    Towers.NumberTheory.Milne.ringOfIntegersGaloisAction
  ...
  letI : IsUltrametricDist w.1.Completion :=
    Towers.NumberTheory.Milne.absoluteUltrametricDist w.1 hwna
  letI : ValuativeRel v.Completion :=
    Towers.CField.Ideles.placeValuativeRel P
  letI : Valuation.Compatible
      (NormedField.valuation (K := v.Completion)) :=
  ...
  letI : IsGalois v.Completion w.1.Completion :=
    Towers.NumberTheory.Milne.placeCompletionGalois v w
  let Aint :=
    Towers.NumberTheory.Milne.completionIntegerRing v
  let Bint :=
    Towers.NumberTheory.Milne.completionIntegerRing w.1
  letI : Algebra Aint v.Completion := Aint.subtype.toAlgebra
  letI : Algebra Aint Bint :=
    Towers.NumberTheory.Milne.completionIntegerLies v w.1 w.2
  ...
  letI : IsIntegralClosure Bint Aint w.1.Completion :=
    Towers.NumberTheory.Milne.completion_integer_closure v w.1 w.2
        (Algebra.IsAlgebraic.of_finite v.Completion w.1.Completion)
  ...
  have hAintVal :
    ...
    ext x
    simp only [Aint, Towers.NumberTheory.Milne.completionIntegerRing,
  ...
  have htauLocalI : tauLocal ∈ ILocal := by
    exact (Towers.TBluepr.decomposition_completion_inertia
    ...
    obstruction_inertia_action
      (betaA := betaA) (hbetaA := hbetaA) Q
        ((decompositionCompletionExtension
          v w.1).symm z.1).1
        ((Towers.TBluepr.decomposition_completion_inertia
  ...
  have heIgen : eI
      (Towers.CField.LBrauer.CyclicH2.generator
        (n := n)) = tauI := by
    simp [eI,
      Towers.CField.LBrauer.CyclicH2.generator]
  ...
  have hsigmaResidue (z : Bint) :
    ...
    have hz :=
      Towers.TBluepr.decomposition_arith_frob
  ...
  let eBaseResidue := centeredCompletionResidue v
    (Towers.CField.Ideles.absolute_value_nontrivial P) hvna
  ...
  letI : ILocal.Normal :=
    Towers.NumberTheory.Milne.inertia_forall_smul
      PInt (fun sigma ↦ MulAction.mem_stabilizer_iff.mp
        (hPIntStable.symm ▸ Subgroup.mem_top sigma))
  let eResidue :=
    Towers.NumberTheory.Milne.gal_stabilizer_top
  ...
  letI : NormedAlgebra v.Completion w.1.Completion := {
    ...
    norm_smul_le := by
      intro a x
      change ‖Towers.NumberTheory.Milne.completionLies
        v w.1 w.2 a * x‖ ≤ ‖a‖ * ‖x‖
      calc
        _ ≤ ‖Towers.NumberTheory.Milne.completionLies
            v w.1 w.2 a‖ * ‖x‖ := norm_mul_le _ _
        _ = ‖a‖ * ‖x‖ := by
          congr 1
          simpa only [dist_zero_right, map_zero] using
            (Towers.NumberTheory.Milne.completion_lies_isometry
  ...
  have hBintVal :
    ...
    ext x
    simp only [Bint, Towers.NumberTheory.Milne.completionIntegerRing,
      Valuation.mem_integer_iff]
  ...
  have hphiPull : Function.Injective phiPull := by
    ...
    apply Units.map_injective
      (Towers.NumberTheory.Milne.completionEmbedding w.1).injective
    exact hzz'
