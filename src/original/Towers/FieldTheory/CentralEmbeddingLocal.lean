...
theorem primitive_dvd_residue
  ...
  let a₀ : k := g₀
  let a : A :=
    Towers.NumberTheory.Milne.teichmullerLift A a₀
  let q := Fintype.card k
  have haq : a ^ q = a := by
    exact sub_eq_zero.mp (by
      simpa [a, q, Polynomial.IsRoot.def] using
        Towers.NumberTheory.Milne.teichmuller_lift_root A a₀)
  ...
  have haResiduePow : IsLocalRing.residue A (a ^ e) =
    ...
    rw [show IsLocalRing.residue A a = a₀ by
      exact Towers.NumberTheory.Milne.residue_teichmullerLift A a₀]
    change ((g₀ : k) ^ e) = 1
  ...
  have haPow : a ^ e = 1 :=
    Towers.NumberTheory.Milne.teichmullerLift_unique A
  ...
  have heOrderDvd : e ∣ orderOf a := by
    ...
    have hresidue : IsLocalRing.residue A a = a₀ :=
      Towers.NumberTheory.Milne.residue_teichmullerLift A a₀
...
theorem arithmetic_frobenius_primitive
  ...
  have hInt : frobInt = powInt :=
    Towers.NumberTheory.Milne.residue_nat_cast
      heUnitN
...
noncomputable def coefficientIntegerUnits
  ...
  (Towers.CField.LFTheory.localInteger F).comp
    (phi.codRestrict
      (Towers.CField.LFTheory.localUnitSubgroup F) (by
        intro c
