...
noncomputable def gradedMagnusComponent
    ...
    Additive
      ...
      Towers.HallTree.AssociativeWordPolynomial ℤ X :=
  (Submodule.subtype
...
noncomputable def associatedGradedBasis :
    Module.Basis
      (Σ n : ℕ, Towers.HallTree.BasicIndex (α := X) (n + 1))
      ℤ
...
noncomputable def associativePBWBasis :
    Module.Basis
      {sequence : List (Towers.HallTree X) //
        Towers.HallTree.OrderedSequence sequence}
      ℤ
      (Towers.HallTree.AssociativeWordPolynomial ℤ X) :=
...
noncomputable def gradedSingletonSequence
    (i : Σ n : ℕ, Towers.HallTree.BasicIndex (α := X) (n + 1)) :
    {sequence : List (Towers.HallTree X) //
      Towers.HallTree.OrderedSequence sequence} :=
  ⟨[Towers.HallTree.indexedBasicTree i.2],
    Towers.HallTree.indexed_tree_sequence i.2⟩
...
theorem associated_graded_basis
  ...
  change
    (IMagnus.lowerCentralBasis (X := X) m).repr
      ...
      Finsupp.single
        (⟨n, i⟩ :
          Σ k : ℕ, Towers.HallTree.BasicIndex (α := X) (k + 1))
        1
        (⟨m, j⟩ :
          Σ k : ℕ, Towers.HallTree.BasicIndex (α := X) (k + 1))
  by_cases h : n = m
...
theorem graded_magnus_basis
  ...
  have hpbw :
      associativePBWBasis (X := X)
        ...
        Towers.HallTree.associativeWordProduct ℤ
          [Towers.HallTree.indexedBasicTree i.2] := by
    simpa [associativePBWBasis,
      gradedSingletonSequence] using
      (Towers.HallTree.PUInput.basis_apply
        ℤ
  ...
  have hdomain :
      domainBasis i.2 =
        (Towers.HallTree.indexedBasicTree i.2).freeLowerWeight
          (Towers.HallTree.indexed_tree_weight i.2) := by
