    ...
    (hrepresentations :
      ∀ xs : List X, xs.length = s →
        wordCoefficientRepresentation
            (R := ZMod modulus) xs g =
          1)
    (w : Towers.TBluepr.AssociativeWordsLength X s) :
    (modulus : ℤ) ∣
...
theorem unitriangular_incidence_bot
  ...
  have hφsurj : Function.Surjective φ := by
    ...
    simp [φ]
  rw [← Towers.Edmonton.central_series_surjective
    φ hφsurj (N - 1)]
...
theorem general_standard_dvd
  ...
  | h s ih =>
      ...
      have hpdiv :
          ∀ w :
              Towers.TBluepr.AssociativeWordsLength
                (FreeGenerator.{u} t) s,
            (generalStandardOrder order i : ℤ) ∣
              (homogeneousPart s
      ...
      let L :
          Towers.TBluepr.AssociativeHomogeneousWords
