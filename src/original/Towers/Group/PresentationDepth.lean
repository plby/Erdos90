...
def FrattiniLinearModel {p : ℕ} (G : Type u) [Group G] : Prop :=
  ∃ V : eFSpace.{u} p,
    Nonempty (V.carrier ≃ₗ[ZMod p] Towers.mFAdditi p G)
...
lemma frattini_mk_pow {p : ℕ} {G : Type u} [Group G] (g : G) :
    ((QuotientGroup.mk' (Towers.Group.fSubgro p G)) g :
      Towers.Group.frattiniQuotient p G) ^ p = 1 := by
  have hmem : g ^ p ∈ Towers.Group.fSubgro p G := by
    change g ^ p ∈ Subgroup.normalClosure
      (Towers.Group.pPowers p G ∪ Towers.Group.commutators G)
    exact Subgroup.subset_normalClosure (by
      left
      exact ⟨g, rfl⟩)
  have hq :
      ((QuotientGroup.mk' (Towers.Group.fSubgro p G)) (g ^ p) :
        Towers.Group.frattiniQuotient p G) = 1 := by
    exact (QuotientGroup.eq_one_iff (N := Towers.Group.fSubgro p G)
      (g ^ p)).mpr hmem
  simpa [map_pow] using hq

lemma frattini_pow_one {p : ℕ} {G : Type u} [Group G]
    (x : Towers.Group.frattiniQuotient p G) :
    x ^ p = 1 := by
...
lemma frattini_mk_comm {p : ℕ} {G : Type u} [Group G] (g h : G) :
    (((QuotientGroup.mk' (Towers.Group.fSubgro p G)) g) *
      ((QuotientGroup.mk' (Towers.Group.fSubgro p G)) h) *
      ((QuotientGroup.mk' (Towers.Group.fSubgro p G)) g)⁻¹ *
      ((QuotientGroup.mk' (Towers.Group.fSubgro p G)) h)⁻¹ :
      Towers.Group.frattiniQuotient p G) = 1 := by
  have hmem : g * h * g⁻¹ * h⁻¹ ∈ Towers.Group.fSubgro p G := by
    change g * h * g⁻¹ * h⁻¹ ∈
      Subgroup.normalClosure (Towers.Group.pPowers p G ∪ Towers.Group.commutators G)
    exact Subgroup.subset_normalClosure (by
      right
      exact ⟨(g, h), rfl⟩)
  have hq :
      ((QuotientGroup.mk' (Towers.Group.fSubgro p G)) (g * h * g⁻¹ * h⁻¹) :
        Towers.Group.frattiniQuotient p G) = 1 := by
    exact (QuotientGroup.eq_one_iff (N := Towers.Group.fSubgro p G)
      (g * h * g⁻¹ * h⁻¹)).mpr hmem
...
lemma frattini_comm_one {p : ℕ} {G : Type u} [Group G]
    (x y : Towers.Group.frattiniQuotient p G) :
    x * y * x⁻¹ * y⁻¹ = 1 := by
  refine Quotient.inductionOn x ?_
  intro g
...
lemma frattini_subgroup_ker {p : ℕ}
    ...
    (φ : A →* Towers.Group.frattiniQuotient p B) :
    Towers.Group.fSubgro p A ≤ φ.ker := by
  change Subgroup.normalClosure (Towers.Group.pPowers p A ∪ Towers.Group.commutators A) ≤ φ.ker
...
lemma frattini_mod_p {p : ℕ} {G : Type u} [Group G] :
    Towers.Group.fSubgro p G = Towers.modPFrattini p G := by
  apply le_antisymm
  · dsimp [Towers.Group.fSubgro, Towers.Group.mAKern]
    refine Subgroup.normalClosure_le_normal ?_
    intro x hx
    rcases hx with hp | hc
    · exact (le_sup_left : Towers.pPowerSubgroup p G ≤ Towers.modPFrattini p G) (by
        dsimp [Towers.pPowerSubgroup, Towers.Group.pPowers] at hp ⊢
        exact Subgroup.subset_normalClosure hp)
    · dsimp [Towers.Group.commutators] at hc
      rcases hc with ⟨ab, rfl⟩
      rcases ab with ⟨a, b⟩
      exact (le_sup_right : _root_.commutator G ≤ Towers.modPFrattini p G)
        (commutator_word (a := a) (b := b))
  · dsimp [Towers.modPFrattini]
    refine sup_le ?hp ?hc
    · dsimp [Towers.pPowerSubgroup]
      refine Subgroup.normalClosure_le_normal ?_
      intro x hx
      dsimp [Towers.Group.fSubgro, Towers.Group.mAKern]
      exact Subgroup.subset_normalClosure (by
        left
        simpa [Towers.Group.pPowers] using hx)
    · rw [_root_.commutator_def]
      ...
      exact Subgroup.subset_normalClosure (by
        right
        change ⁅a, b⁆ ∈ Towers.Group.commutators G
        change (a * b * a⁻¹ * b⁻¹) ∈ Towers.Group.commutators G
        exact ⟨(a, b), rfl⟩)
...
lemma frattini_zassenhaus_two {p : ℕ} {G : Type u} [Group G]
    (_hp : Nat.Prime p) :
    Towers.Group.fSubgro p G ≤ GroupAlgebra.zSubgro p G 2 := by
  classical
...
lemma zassenhaus_frattini {p : ℕ} {G : Type u} [Group G]
    (hp : Nat.Prime p) :
    GroupAlgebra.zSubgro p G 2 ≤ Towers.Group.fSubgro p G := by
  classical
  haveI : NeZero p := ⟨hp.ne_zero⟩
  intro g hg
  let k := ZMod p
  let N : Subgroup G := Towers.Group.fSubgro p G
  haveI : N.Normal := by
    dsimp [N, Towers.Group.fSubgro, Towers.Group.mAKern]
    infer_instance
  let Q := G ⧸ N
...
theorem frattiniFormulaGroups {p : ℕ} {G : Type u} [Group G]
  ...
  let N : Subgroup G := Towers.Group.fSubgro p G
  let Q : Type u := Towers.Group.frattiniQuotient p G
...
theorem d2Phi {p : ℕ} {G : Type u} [Group G]
    (hG : fPGroups p G) :
    GroupAlgebra.zSubgro p G 2 = Towers.Group.fSubgro p G
  := by
  classical
...
theorem frattiniElementaryAbelian {p : ℕ} {G : Type u} [Group G]
  ...
  let V : eFSpace.{u} p :=
    { fact_prime := fPGroups.fact_prime hG
      carrier := Towers.mFAdditi p G
      addCommGroup := (inferInstance : AddCommGroup (Towers.mFAdditi p G))
      module' := (inferInstance : Module (ZMod p) (Towers.mFAdditi p G))
      finite' := (inferInstance : Finite (Towers.mFAdditi p G))
      exponent_p := Towers.mod_additive_nsmul p G }
  refine ⟨V, ?_⟩
  dsimp [V]
  exact ⟨LinearEquiv.refl (ZMod p) (Towers.mFAdditi p G)⟩
/-- A degree-one basis spans the Frattini quotient. -/
theorem burnsideBasis {p : ℕ} {G : Type u} [Group G]
  ...
  have hMtop : M = ⊤ := by
    ...
    have hq' :
        QuotientGroup.mk' (Towers.Group.fSubgro p G) (m⁻¹ * g) = 1 := by
      simpa [frattiniProjection, nSubgro.projection, quotientGroup,
        frattiniNormalSubgroup] using hq
    have hker : m⁻¹ * g ∈ Towers.Group.fSubgro p G :=
      (QuotientGroup.eq_one_iff (m⁻¹ * g)).mp hq'
    have hkerM : m⁻¹ * g ∈ M :=
      hΦM (by simpa [Towers.Group.fSubgro] using hker)
...
theorem degree_generator_rank {p : ℕ} {G : Type u} [Group G]
    (hG : fPGroups p G) :
    degreeGeneratorRank p G = Towers.generatorRank p G := by
  classical
  haveI : Fact p.Prime := ⟨hG.1⟩
  haveI : NeZero p := ⟨hG.1.ne_zero⟩
  have hker : Towers.Group.fSubgro p G = Towers.modPFrattini p G :=
    frattini_mod_p (p := p) (G := G)
  unfold degreeGeneratorRank Towers.generatorRank
  change
    sInf {c : Cardinal | ∃ S : Set (G ⧸ Towers.Group.fSubgro p G),
      Subgroup.closure S = ⊤ ∧ Cardinal.mk S = c}
      = Module.rank (ZMod p) (Towers.mFAdditi p G)
  calc
    sInf {c : Cardinal | ∃ S : Set (G ⧸ Towers.Group.fSubgro p G),
      Subgroup.closure S = ⊤ ∧ Cardinal.mk S = c}
        =
      sInf {c : Cardinal | ∃ S : Set (G ⧸ Towers.modPFrattini p G),
        Subgroup.closure S = ⊤ ∧ Cardinal.mk S = c} :=
          multiplicative_inf_congr
            (QuotientGroup.quotientMulEquivOfEq hker)
    _ = Module.rank (ZMod p) (Towers.mFAdditi p G) := by
          simpa [Towers.mFAdditi, Towers.mFQuot] using
...
theorem equalsFrattiniDimension {p : ℕ} {G : Type u} [Group G]
    ...
    (hV : Nonempty (V.carrier ≃ₗ[ZMod p] Towers.mFAdditi p G)) :
  ...
  have hquot :
      degreeGeneratorRank p G =
        Module.rank (ZMod p) (Towers.mFAdditi p G) := by
...
private lemma frattini_unchanged_aux {p : ℕ}
  ...
  := by
  let ψ0 : G →* Towers.Group.frattiniQuotient p G :=
    QuotientGroup.mk' (Towers.Group.fSubgro p G)
  have hNker : Q.kernel.carrier ≤ ψ0.ker := by
    intro g hg
    change ψ0 g = 1
    dsimp [ψ0]
    exact (QuotientGroup.eq_one_iff (N := Towers.Group.fSubgro p G) g).mpr
      (Q.le_frattini hg)
  let ψ : Towers.Group.quotientGroup Q.kernel →* Towers.Group.frattiniQuotient p G :=
    QuotientGroup.lift Q.kernel.carrier ψ0 hNker
  have hψ :
      Towers.Group.fSubgro p (Towers.Group.quotientGroup Q.kernel) ≤ ψ.ker :=
    frattini_subgroup_ker
      (p := p) (A := Towers.Group.quotientGroup Q.kernel) (B := G) ψ
  let toG :
      Towers.Group.frattiniQuotient p (Towers.Group.quotientGroup Q.kernel) →*
        Towers.Group.frattiniQuotient p G :=
    QuotientGroup.lift
      (Towers.Group.fSubgro p (Towers.Group.quotientGroup Q.kernel)) ψ hψ
  let θ0 : G →* Towers.Group.frattiniQuotient p (Towers.Group.quotientGroup Q.kernel) :=
    (QuotientGroup.mk'
      (Towers.Group.fSubgro p (Towers.Group.quotientGroup Q.kernel))).comp
        (QuotientGroup.mk' Q.kernel.carrier)
  have hθ0 : Towers.Group.fSubgro p G ≤ θ0.ker :=
    frattini_subgroup_ker
      (p := p) (A := G) (B := Towers.Group.quotientGroup Q.kernel) θ0
  let toQ :
      Towers.Group.frattiniQuotient p G →*
        Towers.Group.frattiniQuotient p (Towers.Group.quotientGroup Q.kernel) :=
    QuotientGroup.lift (Towers.Group.fSubgro p G) θ0 hθ0
  refine ⟨
    { toFun := toG
      invFun := toQ
      left_inv := ?_
  ...
  · intro a
    ...
    intro g
    change toQ
        (toG ((QuotientGroup.mk' (Towers.Group.fSubgro p
          (Towers.Group.quotientGroup Q.kernel)))
          ((QuotientGroup.mk' Q.kernel.carrier) g))) =
      (QuotientGroup.mk' (Towers.Group.fSubgro p
        (Towers.Group.quotientGroup Q.kernel)))
        ((QuotientGroup.mk' Q.kernel.carrier) g)
  ...
  · intro b
    ...
    intro g
    change toG (toQ ((QuotientGroup.mk' (Towers.Group.fSubgro p G)) g)) =
      (QuotientGroup.mk' (Towers.Group.fSubgro p G)) g
    dsimp [toG, toQ, ψ, ψ0, θ0]
    rfl
  · intro x y
...
theorem degreeLiftsGenerators {p : ℕ} {G : Type u} [Group G]
  ...
  haveI : Fact p.Prime := ⟨hG.1⟩
  haveI : NeZero p := ⟨hG.1.ne_zero⟩
  let eQ : frattiniQuotient p G ≃* Towers.mFQuot p G :=
    QuotientGroup.quotientMulEquivOfEq
      (frattini_mod_p (p := p) (G := G))
  let basisImage : Set (Towers.mFAdditi p G) :=
    Additive.ofMul '' (eQ '' Set.range B.vector)
  have hbasisClosure_modP :
      Subgroup.closure
          (eQ '' Set.range B.vector : Set (Towers.mFQuot p G)) = ⊤ :=
  ...
  have hbasisImage_span :
    ...
    simpa [basisImage] using
      (span_top_closure (p := p)
        (A := Towers.mFQuot p G)
        (S := (eQ '' Set.range B.vector : Set (Towers.mFQuot p G)))
        hbasisClosure_modP)
  have hbasisImage_linear : LinearIndepOn (ZMod p) id basisImage := by
    ...
    have hother_eq :
        basisImage \ {Additive.ofMul (eQ q)} =
          Additive.ofMul '' (eQ '' T : Set (Towers.mFQuot p G)) := by
      ext y
      constructor
      · intro hy
    ...
    have hspan_T_add :
        Submodule.span (ZMod p)
            (Additive.ofMul '' (eQ '' T : Set (Towers.mFQuot p G))) =
          ⊤ := by
      simpa [hother_eq] using hother_span
    have hclosure_eT :
        Subgroup.closure (eQ '' T : Set (Towers.mFQuot p G)) = ⊤ :=
      closure_top_span (p := p)
        (A := Towers.mFQuot p G)
        (S := (eQ '' T : Set (Towers.mFQuot p G))) hspan_T_add
    have hclosure_T : Subgroup.closure T = ⊤ := by
      have h :=
        closure_image_top eQ.symm hclosure_eT
      have hpreimage : eQ.symm '' (eQ '' T : Set (Towers.mFQuot p G)) = T := by
        ext y
        constructor
  ...
  have hminimal :
    ...
    have hW_span : Submodule.span (ZMod p) W = ⊤ := by
      let imageSet : Set (Towers.mFQuot p G) :=
      ...
      simpa [W, imageSet, hW_eq] using
        (span_top_closure (p := p)
          (A := Towers.mFQuot p G) (S := imageSet)
          hclosure_imageSet)
    have hlinSubtype :
        LinearIndependent (ZMod p)
          ((↑) : basisImage → Towers.mFAdditi p G) := by
