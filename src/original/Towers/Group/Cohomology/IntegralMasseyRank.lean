import Towers.Group.Cohomology.MagnusMassey
import Towers.Group.HallBasic.HallWittFormula
...
noncomputable def integralMagnusBasis
    {n : ℕ} (hn : 1 ≤ n) :
    Module.Basis
      (Towers.HallTree.BasicIndex (α := X) n) ℤ
...
theorem integral_magnus_finrank
    ...
    Module.finrank ℤ
      ...
      Fintype.card (Towers.HallTree.BasicIndex (α := X) n) :=
...
theorem integral_massey_index
    ...
    Module.finrank ℤ
        (ExplicitMasseyImage
          ...
          (lt_of_lt_of_le Nat.zero_lt_two hn)) =
      Fintype.card
        (Towers.HallTree.BasicIndex (α := X) n) := by
  let hnpos : 0 < n :=
    lt_of_lt_of_le Nat.zero_lt_two hn
  calc
    Module.finrank ℤ
    ...
    _ =
        Fintype.card
          (Towers.HallTree.BasicIndex (α := X) n) :=
...
theorem massey_witt_numerator
    ...
    (n : ℤ) *
      ...
      Towers.Edmonton.wittNumerator (Fintype.card X) n := by
  classical
  rw [integral_massey_index
    (X := X) hn]
  exact Towers.HallTree.card_witt_numerator
...
theorem massey_witt_formula
  ...
  rw [integral_massey_index
    (X := X) hn]
  exact Towers.HallTree.card_witt_formula
    (α := X) n (lt_of_lt_of_le Nat.zero_lt_two hn)
...
theorem integral_cup_finrank :
  ...
  rw [integral_massey_index
    (X := X) (n := 2) (by omega)]
  exact Towers.HallTree.card_basic_index
