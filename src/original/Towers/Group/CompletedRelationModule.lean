...
import Towers.Algebra.DenseGenerators.DimensionSubgroup
...
noncomputable def tmp_fox_prod
    {p : ℕ} [Fact p.Prime]
    (Q : Type u) [Group Q] :
    Towers.Theorems.FoxPair (ZMod p) Q ≃
...
noncomputable def tmp_completed_aux
    ...
    (hQ : IsPGroup p Q)
    (j : Fin d) :
    F.Carrier →* Towers.Theorems.FoxPair (ZMod p) Q := by
  letI : DecidableEq Q := Classical.decEq Q
  letI : Fintype Q := Fintype.ofFinite Q
  letI : Fintype (MonoidAlgebra (ZMod p) Q) :=
    show Fintype (Q →₀ ZMod p) from inferInstance
  let P := Towers.Theorems.FoxPair (ZMod p) Q
  letI : Finite P :=
    Finite.of_equiv (MonoidAlgebra (ZMod p) Q × Q)
      (tmp_fox_prod Q).symm
...
theorem tmp_aux_elt
  ...
  let f : ContinuousHom F.Carrier Q :=
    { toMonoidHom :=
        (Towers.Theorems.FoxPair.eltHom (R := ZMod p) (G := Q)).comp
          (F.lift hP gen).toMonoidHom
      continuous_toFun :=
        (show Continuous
            (fun z : P =>
              Towers.Theorems.FoxPair.eltHom (R := ZMod p) (G := Q) z) from
...
noncomputable def tmp_deriv_linear
    {p : ℕ} [Fact p.Prime]
    (Q : Type u) [Group Q] :
    MonoidAlgebra (ZMod p) (Towers.Theorems.FoxPair (ZMod p) Q) →ₗ[ZMod p]
      MonoidAlgebra (ZMod p) Q :=
...
theorem tmp_deriv_single
    {p : ℕ} [Fact p.Prime]
    (Q : Type u) [Group Q]
    (x : Towers.Theorems.FoxPair (ZMod p) Q)
    (a : ZMod p) :
...
theorem tmp_pair_deriv
    {p : ℕ} [Fact p.Prime]
    (Q : Type u) [Group Q]
    (x y : MonoidAlgebra (ZMod p) (Towers.Theorems.FoxPair (ZMod p) Q)) :
    tmp_deriv_linear Q (x * y) =
      GroupAlgebra.augmentation (ZMod p)
          (Towers.Theorems.FoxPair (ZMod p) Q) y •
        tmp_deriv_linear Q x +
      GroupAlgebra.mapGroupHom (ZMod p)
          (Towers.Theorems.FoxPair.eltHom (R := ZMod p) (G := Q)) x *
  ...
  | single a r =>
      ...
      | single b s =>
          ...
          rw [Towers.Theorems.FoxPair.deriv_mul]
          ext q
...
theorem tmp_deriv_pred
    ...
    (Q : Type u) [Group Q]
    {n : ℕ}
    {x : MonoidAlgebra (ZMod p) (Towers.Theorems.FoxPair (ZMod p) Q)}
    (hx :
      x ∈
        (GroupAlgebra.augmentationIdeal
          (ZMod p) (Towers.Theorems.FoxPair (ZMod p) Q)) ^ n) :
    tmp_deriv_linear Q x ∈
      (GroupAlgebra.augmentationIdeal (ZMod p) Q) ^ (n - 1) := by
  classical
  let IP : Ideal
      (MonoidAlgebra (ZMod p) (Towers.Theorems.FoxPair (ZMod p) Q)) :=
    GroupAlgebra.augmentationIdeal
      (ZMod p) (Towers.Theorems.FoxPair (ZMod p) Q)
  ...
  | succ n =>
      ...
      have htarget : tmp_deriv_linear Q x ∈ IQ ^ n := by
        ...
        · intro a ha b hb
          ...
          have haug :
              GroupAlgebra.augmentation (ZMod p)
                (Towers.Theorems.FoxPair (ZMod p) Q) b = 0 := by
            exact
              (GroupAlgebra.mem_augmentationIdeal
                (R := ZMod p)
                (G := Towers.Theorems.FoxPair (ZMod p) Q)).mp
                (by simpa [IP] using hb)
          rw [haug, zero_smul, zero_add]
          have hmap :
              GroupAlgebra.mapGroupHom (ZMod p)
                  (Towers.Theorems.FoxPair.eltHom (R := ZMod p) (G := Q)) a ∈
                IQ ^ n := by
            simpa [IQ, IP] using
              GroupAlgebra.group_augmentation_power
                (ZMod p)
                (Towers.Theorems.FoxPair.eltHom (R := ZMod p) (G := Q))
...
theorem tmp_fox_deriv
    ...
    tmp_deriv_linear Q
        (MonoidAlgebra.of (ZMod p)
          (Towers.Theorems.FoxPair (ZMod p) Q) x - 1) =
      x.deriv := by
  change
    tmp_deriv_linear Q
        (Finsupp.single x 1 - Finsupp.single (1 :
          Towers.Theorems.FoxPair (ZMod p) Q) 1) =
      x.deriv
  calc
    tmp_deriv_linear Q
          (Finsupp.single x 1 - Finsupp.single (1 :
        ...
        tmp_deriv_linear Q (Finsupp.single x 1) -
          tmp_deriv_linear Q
            (Finsupp.single (1 : Towers.Theorems.FoxPair (ZMod p) Q) 1) := by
