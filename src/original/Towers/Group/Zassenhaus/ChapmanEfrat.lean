import Towers.Group.Zassenhaus.ArbitraryRankDescent
import Towers.Group.DenseGenerators.ZassenhausJenningsReduction
...
theorem explicit_filtration_dimension
    [Fact p.Prime] (n : ℕ) :
    _root_.Towers.zassenhausFiltration p G n ≤ dSubgro (ZMod p) G n := by
  exact _root_.Towers.filtration_dimension_zmod
    (p := p) (Q := G) n
...
theorem explicit_filtration_laws
    ...
    (hsucc :
      ∀ {Q : Type u} [Group Q] [Finite Q],
        _root_.Towers.zassenhausFiltration p Q n = ⊥ →
      ...
      ∀ {Q : Type u} [Group Q] [Finite Q],
        _root_.Towers.zassenhausFiltration p Q n = ⊥ →
          ∀ {r s : ℕ} {x y : Q},
            r < n →
            s < n →
            x ∈ _root_.Towers.exactGeneratorSet p Q r →
            y ∈ _root_.Towers.exactGeneratorSet p Q s →
              ⁅x, y⁆ ∈ _root_.Towers.zassenhausFiltration p Q (r + s)) :
    _root_.Towers.zassenhausFiltration p G n =
        ...
        _root_.Towers.zassenhausFiltration p Q n = ⊥ →
          ∀ {r s : ℕ} {x y : Q},
            ...
            y ∈ _root_.Towers.exactGeneratorSet p Q s →
              ⁅x, y⁆ ∈ _root_.Towers.zassenhausFiltration p Q (r + s))
    (g : G) :
    g ∈ _root_.Towers.zassenhausFiltration p G n ↔
      (_root_.MonoidAlgebra.of (ZMod p) G g - 1 :
        _root_.MonoidAlgebra (ZMod p) G) ∈
        augmentationPower (ZMod p) G n := by
...
/-- The explicit lower-central-product Zassenhaus filtration used in `Towers`
...
theorem explicit_logarithmic_product
    ...
    _root_.Towers.zassenhausFiltration p G n =
      EChapma.logarithmicLowerProduct (G := G) p 1 hp n := by
  apply le_antisymm
  · rw [_root_.Towers.zassenhausFiltration, Subgroup.closure_le]
    intro g hg
  ...
  · unfold EChapma.logarithmicLowerProduct
    apply iSup_le
    intro idx
    letI : (_root_.Towers.zassenhausFiltration p G n).Normal :=
      _root_.Towers.zassenhausFiltration_normal p G n
...
theorem chapman_efrat_criterion
    (p n : ℕ) (g : G) :
    g ∈ _root_.Towers.GroupAlgebra.zSubgro p G n ↔
      (_root_.MonoidAlgebra.of (ZMod p) G g - 1 :
        _root_.MonoidAlgebra (ZMod p) G) ∈
        _root_.Towers.GroupAlgebra.augmentationPower (ZMod p) G n :=
  GroupAlgebra.mod_p_sub
    (p := p) (G := G) n g

/--
Paper-facing `q = p` Chapman--Efrat lower-central product formula for the
explicit Zassenhaus filtration used by `Towers`.
-/
...
the Towers dimension-subgroup API.
-/
