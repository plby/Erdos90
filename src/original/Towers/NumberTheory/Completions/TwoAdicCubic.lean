...
private theorem cubic_mixed_independent :
  ...
  have hc : ∑ j, c j • b j = 0 := by
    ...
    simp only [Fin.val_zero, pow_zero, Fin.val_one, pow_one, Fin.val_two,
      Algebra.smul_def]
    have hmapdiv (q : ℚ) :
        algebraMap ℚ AdicCubicField (q / 2) =
          algebraMap ℚ AdicCubicField q /
            algebraMap ℚ AdicCubicField 2 := by
      exact map_div₀ (algebraMap ℚ AdicCubicField) q 2
    rw [map_add, hmapdiv]
    norm_num only [map_ofNat]
    have hmap (q : ℚ) :
        algebraMap ℚ AdicCubicField q = (q : AdicCubicField) := by
