...
import Towers.ClassField.LocalExistence.AssembledLocalReciprocity
...
import Towers.ClassField.UnramifiedCohom.NormInteger
...
theorem absolute_spectral_restriction
    ...
    v.IsEquiv
      (Towers.CField.LBrauer.spectralAbsoluteValue K L) := by
  obtain ⟨c, hc, hpow⟩ :=
  ...
  have hw (x : L) : w x =
      Towers.CField.LBrauer.spectralAbsoluteValue K L x := by
...
theorem alg_unramified_level
    ...
    (hE : Towers.CField.UCohom.FUExt.IsUnramified K E) :
  ...
  letI : IsGalois K E :=
    Towers.CField.UCohom.FUExt.galoisUnramified
      K E hE
  ...
  have hE' : Module.Finite A₀ U ∧ Algebra.FormallyUnramified A₀ U := by
    simpa [Towers.CField.UCohom.FUExt.IsUnramified,
      A₀, U] using hE
...
theorem basic_witt_cardinality
  ...
  letI : Module.Finite A OR :=
    Towers.NumberTheory.Milne.valued_integer_module ℚ_[p] R
  ...
  have htotal : Towers.NumberTheory.Milne.TotallyRamified
    ...
    let hI : IsAdic (IsLocalRing.maximalIdeal OR) :=
      Towers.NumberTheory.Milne.valued_integer_adic R
  ...
  have hcardR : localResidueCard R = localResidueCard ℚ_[p] := by
    ...
    have hbij : Function.Bijective (algebraMap (A ⧸ pA) (OR ⧸ P)) :=
      Towers.NumberTheory.Milne.bijective_full_idx
...
theorem basic_comparison_order (n : ℕ) (u : ℤ_[p]ˣ) :
  ...
  apply (Towers.CField.NCorr.local_element_order
