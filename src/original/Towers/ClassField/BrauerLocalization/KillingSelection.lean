...
import Towers.ClassField.BrauerLocalization.CardinalityDirect
...
def SelectedSpectralFormula
  ...
  letI : FiniteDimensional v.Completion
    ...
    Towers.NumberTheory.Milne.placeCompletionDimensional
...
theorem change_totally_complex
    ...
    (completion : HasseCompletionData K L)
    (v : InfinitePlace K)
    (x : BrauerGroup (Towers.CField.RExist.placeCompletion K (.inr v))) :
