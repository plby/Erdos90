import Submission.ClassField.LocalGlobalPowers.Counterexample
...
namespace Submission.CField.LGPowers.EEPower

open Submission.CField.LGPowers
...
theorem sixteen_eighth_real :
    ∃ x : ℝ, x ^ 8 = 16 :=
  Submission.CField.LGPowers.sixteen_eighth_real

/-- **Exercise A-11, rational obstruction.** `16` is not an eighth power in
`Q`. -/
theorem sixteen_eighth_rational :
    ¬∃ x : ℚ, x ^ 8 = 16 :=
  Submission.CField.LGPowers.sixteen_eighth_rational
...
end Submission.CField.LGPowers.EEPower
