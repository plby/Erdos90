...
theorem change_nonidentity_relative
    ...
    [∀ v, Field (Lv v)]
    [∀ v, Algebra (Submission.CField.RExist.placeCompletion K v) (Lv v)]
    [∀ v, IsGalois (Submission.CField.RExist.placeCompletion K v) (Lv v)]
    (loc : RelativeLocalizationData (K := K) (L := L)
      (fun v ↦ Submission.CField.RExist.placeCompletion K v) Lv)
  ...
  intro v hv
  have hcoord :
      ((y v).toMul : BrauerGroup (Submission.CField.RExist.placeCompletion K v)) =
        brauerBaseChange K (Submission.CField.RExist.placeCompletion K v) beta := by
...
def GlobalChangeSupport
  ...
  ∀ beta : BrauerGroup K, Set.Finite <|
    {v : NumberFieldPlace K |
      brauerBaseChange K (Submission.CField.RExist.placeCompletion K v) beta ≠ 1}
...
noncomputable def globalBrauerLocalization
    ...
    MultiplicativeLocalizationData (BrauerGroup K)
      (fun v : NumberFieldPlace K ↦
        BrauerGroup (Submission.CField.RExist.placeCompletion K v)) :=
  multiplicativeLocalizationSupport
    (fun v ↦ brauerBaseChange K (Submission.CField.RExist.placeCompletion K v)) hfinite
