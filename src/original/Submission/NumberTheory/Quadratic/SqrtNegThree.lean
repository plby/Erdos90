...
def toEisenstein : SNThree →+*
    Submission.NumberTheory.EInts where
...
theorem omega_range_eisenstein :
    Submission.NumberTheory.EInts.omega ∉
...
noncomputable instance fractionRingAlgebra :
    Algebra SNThree
      (FractionRing Submission.NumberTheory.EInts) :=
  ((algebraMap Submission.NumberTheory.EInts
      (FractionRing Submission.NumberTheory.EInts)).comp
    toEisenstein).toAlgebra

noncomputable instance fraction_faithful_s :
    FaithfulSMul SNThree
      (FractionRing Submission.NumberTheory.EInts) := by
  ...
  exact (IsFractionRing.injective
    Submission.NumberTheory.EInts _).comp
...
noncomputable instance fraction_sqrt_neg :
  ...
  obtain ⟨a, b, hb, hab⟩ := IsFractionRing.div_surjective
    Submission.NumberTheory.EInts z
  refine ⟨clearTwo a, clearTwo b, ?_⟩
  change
    z = algebraMap Submission.NumberTheory.EInts _
          (toEisenstein (clearTwo a)) /
        algebraMap Submission.NumberTheory.EInts _
          (toEisenstein (clearTwo b))
  rw [eisenstein_clear_two, eisenstein_clear_two, map_mul, map_mul]
  have htwo :
      algebraMap Submission.NumberTheory.EInts
        (FractionRing Submission.NumberTheory.EInts) 2 ≠ 0 := by
    rw [map_ne_zero_iff _ (IsFractionRing.injective
      Submission.NumberTheory.EInts _)]
    norm_num
...
noncomputable def fractionRingEquiv :
  ...
  (FractionRing.algEquiv SNThree
    (FractionRing Submission.NumberTheory.EInts)).toRingEquiv
