...
import Towers.Group.DenseGenerators.ZassenhausJenningsInduction
...
theorem formula_closure_filtration {p : ℕ} {G : Type u} [Group G]
    ...
    zFTerm p n G = _root_.Towers.zassenhausFiltration p G n := by
  apply le_antisymm
  · haveI : (_root_.Towers.zassenhausFiltration p G n).Normal :=
  ...
  · rw [_root_.Towers.zassenhausFiltration]
...
lemma exact_killed_boundary
    ...
    ∀ {Q : Type u} [Group Q] [Finite Q],
      _root_.Towers.zassenhausFiltration p Q n = ⊥ →
        _root_.Towers.KilledBoundaryTrivial p Q n := by
...
lemma closure_filtration_boundary
    ...
    GroupAlgebra.zSubgro p G n ≤
      _root_.Towers.zassenhausFiltration p G n := by
  change
    GroupAlgebra.dSubgro (ZMod p) G n ≤
      _root_.Towers.zassenhausFiltration p G n
  apply
    _root_.Towers.zmod_boundary_trivial
      (p := p) ?_ hn
...
lemma killed_separation_laws
  ...
  intro x hx
  apply
    _root_.Towers.zmod_generator_law.{u}
      (p := p) (n := n) hnOne
  · intro R _ _ hroot r s a b hr hs ha hb
    have hformulaR : zFTerm p n R = ⊥ := by
      ...
      exact hroot
    have hrPos : 0 < r :=
      _root_.Towers.exact_set_pos ha
    have hsPos : 0 < s :=
      _root_.Towers.exact_set_pos hb
    rw [← formula_closure_filtration
    ...
    exact hcomm hformulaR hr hs
      (by
        rw [formula_closure_filtration
          (p := p) (G := R) r hrPos]
        exact
          _root_.Towers.exact_subset_filtration
            ha)
      (by
        rw [formula_closure_filtration
          (p := p) (G := R) s hsPos]
...
lemma killed_formula_separation
  ...
  intro x hx
  apply
    _root_.Towers.zmod_filtration_bot
      (p := p) (n := n) hn
