...
structure PresentedDepthRefinement
  ...
  highDegree_surjective :
    ∀ n, 2 ≤ n →
      ∃ Ψ :
        _root_.Towers.TBluepr.presentedHighDegree
            (p := p) (d := d) (r := r) rels refinedDepth n →ₗ[ZMod p]
          _root_.Towers.TBluepr.pGTarget
            (p := p) (d := d) (r := r) rels n,
...
theorem PDData.generatorMap_surjective
    ...
    (n : ℕ) (hn : 2 ≤ n) :
    Function.Surjective
      (_root_.Towers.TBluepr.presentedHighMultiplication
...
theorem high_module_surjective
    ...
    d * _root_.Towers.TBluepr.presentedHilbertSequence
        (p := p) rels (n - 1) ≤
      _root_.Towers.TBluepr.presentedHilbertSequence
          (p := p) rels n +
        ∑ i, if depth i ≤ n then
          _root_.Towers.TBluepr.presentedHilbertSequence
            (p := p) rels (n - depth i)
  ...
  classical
  exact
    _root_.Towers.TBluepr.high_bound_surjective
      (p := p) (d := d) (r := r)
      rels depth n Ψ hΨ
...
theorem presented_inequality_refinement
  ...
  · subst n
    simpa using
      _root_.Towers.TBluepr.presented_full_inequality
        (p := p) (d := d) (r := r)
