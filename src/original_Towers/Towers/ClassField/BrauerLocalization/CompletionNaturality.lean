...
private theorem res_coind_iso
  ...
  have hcoind :=
    Towers.CField.Shifting.coindIso_naturality f n
  calc
    ...
    _ = groupCohomology.map (MonoidHom.id G) unit n ≫
      ...
      simpa [coindf] using congrArg
        ...
        hcoind.symm
    _ = Towers.CField.COps.shapiroRestriction
          A H n ≫ groupCohomology.map (MonoidHom.id H) f n := by
      rfl
    _ = Towers.CField.COps.restriction
          A H n ≫ groupCohomology.map (MonoidHom.id H) f n := by
      rw [Towers.CField.COps.restriction_shapiro]
    _ = groupCohomology.map H.subtype f n := by
      rw [Towers.CField.COps.restriction,
