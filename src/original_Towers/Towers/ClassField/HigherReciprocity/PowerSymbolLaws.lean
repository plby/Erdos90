...
theorem split_conic_norm (a b : K) :
    (Towers.CField.QForms.quadraticHilbertSign a b = 1 ↔
      Towers.CField.HSymbol.NontrivialQuadraticConic a b) ∧
    (¬ IsSquare a →
      (Towers.CField.QForms.quadraticHilbertSign a b = 1 ↔
        Towers.CField.HSymbol.QuadraticValue a b)) :=
  ⟨Towers.CField.QForms.hilbert_sign_one a b,
    fun ha ↦
