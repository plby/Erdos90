...
import Submission.Group.Edmonton.VerbalSubgroups
...
lemma rel_index_nilpotent
  ...
  have hpow :=
    Submission.Edmonton.Subgroup.IsSubnormal.pow_index_mem hsubnormal
      (⟨x, hx⟩ : H)
  exact hpow
...
lemma omega_number_mul
  ...
  have hpowQ : (xP * yP) ^ Q.index ∈ Q :=
    Submission.Edmonton.Subgroup.IsSubnormal.pow_index_mem
      hQsubnormal (xP * yP)
...
lemma omega_isolator_compl
  ...
  exact Submission.Edmonton.Subgroup.mem_coprime_powmem S
