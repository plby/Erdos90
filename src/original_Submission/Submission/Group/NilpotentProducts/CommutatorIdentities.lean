import Submission.Group.HallBasic.FreeNilpotentTorsion
import Submission.Group.Edmonton.CommutatorIdentities
...
`(x,y) = x⁻¹ y⁻¹ x y`; this is `Submission.Edmonton.hallCommutator`.
-/
...
theorem cyclic_triple_commutator
    (hG : Submission.Edmonton.Group.IsMetabelian G)
    (x y z : G) :
