...
import Submission.ClassField.KummerTheory.KummerCorrespondence
import Submission.ClassField.GrunwaldWang.CyclicDivision
import Submission.ClassField.GrunwaldWang.GrunwaldWangStatement
import Submission.ClassField.GrunwaldWang.SimultaneousStatement
...
import Submission.FieldTheory.CentralEmbeddingDescent
import Submission.FieldTheory.CentralFactorSet
import Submission.FieldTheory.CentralEmbeddingKummer
import Submission.FieldTheory.CentralSeparableClosure
...
import Submission.FieldTheory.FiniteCompositumUnramified
...
theorem rational_change_not
    ...
    Submission.CField.BGroups.brauerBaseChange Kc v.Completion
...
theorem no_wang_exception
    ...
    [Field K]
    [NumberField K] :
    ¬Submission.CField.GWang.HasWangException K 3 := by
...
theorem grunwald_wang_three
    ...
    [Field K]
    [NumberField K]
    (hGW : Submission.CField.GWang.GrunwaldWangTheorem K)
    (places : Finset
      (Submission.CField.GWang.Place K))
    (localCharacter : ∀ v : places,
      Submission.CField.GWang.OrderLocalCharacter
        K v.1)
    (horder :
      Finset.univ.lcm (fun v : places => orderOf (localCharacter v).1) = 3) :
    ∃ chi : Submission.CField.GWang.IdeleClassCharacter K,
      orderOf chi = 3 ∧
        ∀ v : places,
          Submission.CField.GWang.CharacterRestrictsTo
            K chi v.1 (localCharacter v).1 := by
  let n := Finset.univ.lcm
    (fun v : places => orderOf (localCharacter v).1)
  have hn : n = 3 := horder
  have hno :
      ¬Submission.CField.GWang.HasWangException K n := by
...
theorem reciprocity_law_idele
    ...
    [NumberField K]
    (hreciprocity :
      Submission.CField.Recip.IdeleReciprocityLaw
        (K := K)) :
    Submission.CField.RExist.GlobalReciprocityLaw K := by
  intro phi hphi
...
theorem brauer_class_completions
    ...
    [NumberField K]
    (loc :
      Submission.CField.GClass.GlobalLocalizationData K)
    (placeInvariant :
      ∀ place : Submission.CField.Ideles.NumberFieldPlace K,
        Additive (BrauerGroup
            (Submission.CField.Ideles.placeCompletion
              K place)) →+
          Submission.CField.LBrauer.LocalInvariant)
    (hsequence :
      Submission.CField.GClass.GlobalBrauerSequence
    ...
    (hlocal :
      ∀ place : Submission.CField.Ideles.NumberFieldPlace K,
        Submission.CField.BGroups.brauerBaseChange
            K
            (Submission.CField.Ideles.placeCompletion K place)
            x =
          1) :
    x = 1 := by
  ...
  change
    DirectSum.component ℤ
        (Submission.CField.Ideles.NumberFieldPlace K)
        (fun v => Additive (BrauerGroup
          (Submission.CField.Ideles.placeCompletion K v)))
