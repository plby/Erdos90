import Submission.Algebra.Magnus.IntegralMagnus
import Submission.Algebra.Linear.TorsionFreeCokernel
...
theorem standard_sequence_injective
    ...
    (n : ℕ) :
    Function.Injective
      (Submission.HallTree.orderedStandardSequence
        (α := X) ℤ n) := by
  classical
  letI : Fintype X := Fintype.ofFinite X
  intro left right heq
  have hglobal :
      Submission.HallTree.standardSequenceInclusion
          (α := X) ℤ n left =
        Submission.HallTree.standardSequenceInclusion
          (α := X) ℤ n right := by
    apply
      (IMagnus.hallPBWInput
        (X := X)).eval_injective
    rw [
      Submission.HallTree.standard_sequence_inclusion,
      Submission.HallTree.standard_sequence_inclusion]
...
noncomputable def homogeneousPBWLinear
    (n : ℕ) :
    Submission.HallTree.orderedSequenceSubmodule
        (α := X) ℤ n ≃ₗ[ℤ]
  ...
  LinearEquiv.ofBijective
    ...
    ⟨standard_sequence_injective
        (X := X) n,
      Submission.HallTree.standard_sequence_surjective
        (α := X) ℤ n⟩
...
noncomputable def homogeneousPBWBasis
    ...
    Module.Basis
      {sequence : List (Submission.HallTree X) //
        Submission.HallTree.OrderedStandardSequence n sequence}
      ℤ
      (AssociativeHomogeneousWords ℤ X n) :=
  (Submission.HallTree.standardSequenceBasis
      (α := X) ℤ n).map
...
theorem ordered_standard_sequence
  ...
  apply Subtype.ext
  simpa [
    Submission.HallTree.standardSequenceBasis] using
...
theorem homogeneous_pbw_basis
  ...
  change
    Finsupp.linearCombination ℤ
        ...
        (Finsupp.single sequence.1 1) =
      Submission.HallTree.associativeWordProduct ℤ sequence.1
  simp [            ]
...
noncomputable def basicSingletonSequence
    {n : ℕ}
    (i : Submission.HallTree.BasicIndex (α := X) n) :
    {sequence : List (Submission.HallTree X) //
      Submission.HallTree.OrderedStandardSequence n sequence} :=
  ⟨[Submission.HallTree.indexedBasicTree i],
    Submission.HallTree.indexed_tree_sequence i,
    by simp [Submission.HallTree.indexed_tree_weight]⟩
...
theorem pbw_singleton_sequence
    ...
    augmentationPBWBasis (X := X) n
      ...
      Submission.HallTree.freeMagnusInt X hn
  ...
  unfold
    Submission.HallTree.associativeRealizationWeight
  congr 1
  apply Subtype.ext
  simp [basicSingletonSequence,
    Submission.HallTree.associativeHomogeneousRep,
    Submission.HallTree.associativeRepWeight]
...
theorem free_magnus_range
  ...
  have hfamily :
      (fun i : Submission.HallTree.BasicIndex (α := X) n =>
        Submission.HallTree.freeMagnusInt X hn
          (domainBasis i)) =
        (fun i =>
          augmentationPBWBasis (X := X) n
