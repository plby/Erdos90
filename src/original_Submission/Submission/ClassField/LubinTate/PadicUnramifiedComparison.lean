...
theorem padic_local_card :
    Submission.CField.LFTheory.localResidueCardinality ℚ_[p] = p := by
...
theorem padic_integer_card
    ...
    letI : NormedAlgebra ℚ_[p] E := spectralNorm.normedAlgebra ℚ_[p] E
    letI : IsUltrametricDist E := IsUltrametricDist.of_normedAlgebra ℚ_[p]
    Submission.CField.LFTheory.localResidueCardinality
      E = p := by
  ...
  calc
    ...
    _ = Submission.CField.LFTheory.localResidueCardinality ℚ_[p] := rfl
...
theorem padic_residue_card
    ...
    letI : NormedAlgebra ℚ_[p] E := spectralNorm.normedAlgebra ℚ_[p] E
    letI : IsUltrametricDist E := IsUltrametricDist.of_normedAlgebra ℚ_[p]
    Submission.CField.LFTheory.localResidueCardinality E = p := by
  let E₀ := (lubinTateDatum p u).RootField ℚ_[p] n
  let E := (padicTateDatum p u).RootField ℚ_[p] n
  let e : E₀ ≃ₐ[ℚ_[p]] E :=
...
theorem witt_comparison_arithmetic
    ...
    ∀ x : E, ‖x‖ ≤ 1 →
      ‖wittComparisonFrobenius p k n u x -
          x ^ Submission.CField.LFTheory.localResidueCardinality F₀‖ < 1 := by
  let F₀ := F p k n u
  let E := M p k n u
  letI : Algebra.IsAlgebraic ℚ_[p] F₀ :=
    Algebra.IsAlgebraic.of_finite ℚ_[p] F₀
  ...
  letI : NormedAlgebra ℚ_[p] E := spectralNorm.normedAlgebra ℚ_[p] E
  letI : IsUltrametricDist E := IsUltrametricDist.of_normedAlgebra ℚ_[p]
  have hcard : Submission.CField.LFTheory.localResidueCardinality F₀ = p :=
    padic_comparison_card p k n u
  change ∀ x : E, spectralNorm ℚ_[p] E x ≤ 1 →
    spectralNorm ℚ_[p] E
      (wittComparisonFrobenius p k n u x -
        x ^ Submission.CField.LFTheory.localResidueCardinality F₀) < 1
  intro x hx
