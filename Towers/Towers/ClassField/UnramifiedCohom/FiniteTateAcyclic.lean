...
theorem units_tate_acyclic
  ...
  have hzero : Subsingleton (tateCohomologyZero A) := by
    have hnorm : Function.Surjective
        (Towers.CField.LFTheory.normOnUnits k l) :=
      units_norm_surjective k l
    have hrange : Towers.CField.LFTheory.normSubgroup k l = ⊤ :=
      MonoidHom.range_eq_top.mpr hnorm
    have hquot : Subsingleton
        (kˣ ⧸ Towers.CField.LFTheory.normSubgroup k l) :=
      QuotientGroup.subsingleton_iff.mpr hrange
    letI : Subsingleton
        (kˣ ⧸ Towers.CField.LFTheory.normSubgroup k l) := hquot
    letI : Subsingleton
        (Additive (kˣ ⧸ Towers.CField.LFTheory.normSubgroup k l)) :=
      inferInstance
    exact
