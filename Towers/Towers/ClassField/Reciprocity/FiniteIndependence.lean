...
import Towers.ClassField.LocalReciprocity.UniverseContinuity
import Towers.ClassField.LocalReciprocity.UniverseRestriction
import Towers.ClassField.NormCorrespondence.MaximalAbelianTower
import Towers.ClassField.Reciprocity.UniverseArtinContinuity
import Towers.ClassField.Reciprocity.CompletionArtinHom
import Towers.ClassField.Reciprocity.FiniteProductContinuity
import Towers.ClassField.Reciprocity.FiniteRestrictionTopology
import Towers.ClassField.NormIndex.IdeleTowerLocal
import Towers.ClassField.NormIndex.InfiniteIdeleCompatibility
...
private theorem finiteNormalization
    ...
    let w := (Towers.CField.NIndex.placesAboveFactors
      (K := K) (L := L.1) P).symm Q
...
private theorem infinite_local_restriction
  ...
  · have htComplex : t.1.IsComplex := by
    ...
    have hdegree : Module.finrank F E = 1 :=
      (Towers.CField.GWang.infiniteDegreeCompatibility
...
private theorem chosenPrime_unramified
    ...
    let Q := Towers.CField.NIndex.placeUpperFactor
      (K := K) (L := L.1) P w
...
private theorem local_hom_units
  ...
  have hwq : w.1.IsEquiv
    ...
    change ((Towers.CField.NIndex.placesAboveFactors
      (K := K) (L := L.1) P).symm Q).1.IsEquiv
        (FinitePlace.mk (upperPrime (K := K) (L := L.1) P Q)).val at h
    rw [show (Towers.CField.NIndex.placesAboveFactors
        (K := K) (L := L.1) P).symm Q = w by
      exact (Towers.CField.NIndex.placesAboveFactors
        (K := K) (L := L.1) P).symm_apply_apply w] at h
    exact h
  have hnormRange :=
    Towers.CField.GWang.completion_norm_range
...
private theorem layer_product_local
  ...
  · intro P Q
    let wQ := (Towers.CField.NIndex.placesAboveFactors
      (K := K) (L := L.1) P).symm Q
...
private theorem pow_infinite_subgroup
  ...
  · rw [infiniteBasicSubgroup, dif_neg hv] at hx
    ...
    obtain ⟨z, hz⟩ :=
      Towers.CField.KNIndex.complex_monoid_surjective
        n hn (e x)
    refine ⟨e.symm z, ?_⟩
    apply e.injective
