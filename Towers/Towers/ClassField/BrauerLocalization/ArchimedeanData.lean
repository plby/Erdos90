...
def complexPlaceInvariant
    ...
    (v : InfinitePlace K) :
    Additive (BrauerGroup (Towers.CField.RExist.placeCompletion K (.inr v))) →+
      LocalInvariant := 0
...
theorem complex_invariant_canonical
  ...
  letI : Subsingleton
      (BrauerGroup (Towers.CField.RExist.placeCompletion K (.inr v))) := by
...
noncomputable def realInvariantAdd
    (K : Type u) [Field K] [NumberField K]
    (v : InfinitePlace K)
    (e : Additive (BrauerGroup (Towers.CField.RExist.placeCompletion K (.inr v))) ≃+
      ZMod 2) :
    Additive (BrauerGroup (Towers.CField.RExist.placeCompletion K (.inr v))) →+
      LocalInvariant :=
