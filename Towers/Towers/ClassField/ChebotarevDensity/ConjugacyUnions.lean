...
import Towers.ClassField.ChebotarevDensity.Density
import Towers.ClassField.DirichletDensity.PolarLogBridge
...
noncomputable def conjugacyClassesContained (C : Set G) :
  ...
  classical
  exact Towers.NumberTheory.Milne.conjugacyClassesSatisfying
...
theorem arithmetic_classes_contained
  ...
  simp only [primesArithmeticFrobenius,
    ...
    Towers.NumberTheory.Milne.conjugacyClassesSatisfying,
    ...
    ∑ D ∈ conjugacyClassesContained C, D.carrier.ncard =
      ...
      by
        simpa only [conjugacyClassesContained] using
          (Towers.NumberTheory.Milne.conjugacy_satisfying_ncard
            (fun D : ConjClasses G => D.carrier ⊆ C))
    _ = Nat.card C := by
