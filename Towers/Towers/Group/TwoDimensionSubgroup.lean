import Towers.Group.ZassenhausFrattini
...
theorem p_commutator_aux :
    Subgroup.map (QuotientGroup.mk' (_root_.commutator G))
        (_root_.Towers.pPowerSubgroup p G) =
      _root_.Towers.pPowerSubgroup p (G ⧸ _root_.commutator G) := by
  let q := QuotientGroup.mk' (_root_.commutator G)
  apply le_antisymm
  · rw [Subgroup.map_le_iff_le_comap]
    apply Subgroup.normalClosure_le_normal
    rintro _ ⟨g, rfl⟩
    change q (g ^ p) ∈ _root_.Towers.pPowerSubgroup p (G ⧸ _root_.commutator G)
    simpa using _root_.Towers.p_power_subgroup p (G ⧸ _root_.commutator G) (q g)
  · haveI :
        (Subgroup.map q (_root_.Towers.pPowerSubgroup p G)).Normal :=
    ...
    simpa using
      (Subgroup.mem_map_of_mem q (_root_.Towers.p_power_subgroup p G g))

theorem abelianization_preimage_aux :
    Subgroup.comap (QuotientGroup.mk' (_root_.commutator G))
        (_root_.Towers.pPowerSubgroup p (G ⧸ _root_.commutator G)) =
      _root_.Towers.modPFrattini p G := by
  rw [← p_commutator_aux,
    QuotientGroup.comap_map_mk', sup_comm]
  rfl

theorem zassenhaus_frattini_aux :
    zSubgro p G 2 ≤ _root_.Towers.modPFrattini p G := by
  apply zassenhaus_subgroup_bot p G
  letI : IsMulCommutative (G ⧸ _root_.Towers.modPFrattini p G) :=
    ⟨⟨fun x y => _root_.Towers.mod_mul_comm p G x y⟩⟩
  apply le_antisymm
  · exact _root_.Towers.Theorems.bot_comm_exponent
      p (_root_.Towers.mod_frattini_one p G)
  · exact bot_le

theorem p_two_aux :
    _root_.Towers.pPowerSubgroup p G ≤ zSubgro p G 2 := by
  apply Subgroup.normalClosure_le_normal
  rintro _ ⟨g, rfl⟩
  exact pth_subgroup_two g

theorem mod_frattini_aux :
    _root_.Towers.modPFrattini p G ≤ zSubgro p G 2 := by
  exact sup_le p_two_aux
    commutator_zassenhaus_two

theorem p_frattini_aux :
    zSubgro p G 2 = _root_.Towers.modPFrattini p G :=
...
theorem bot_forall_commute
  ...
  exact le_antisymm
    (_root_.Towers.Theorems.bot_comm_exponent p hpow)
    bot_le
